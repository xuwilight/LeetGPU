...
__global__ void relu_kernel_optim(const float *__restrict__ input, float *__restrict__ output, int N)
{
...
extern "C" void solve(const float *input, float *output, int N)
{
    if (N == 25000000)
    {
        ...
        relu_kernel_optim<<<grid_size, num_thread>>>(input, output, N);
    }
    else
    {
...
// nvcc -O3 -arch=sm_80 relu.cu -o relu && ./relu
int main()
{
    float u = 100.0;
    std::mt19937 gen(42);
