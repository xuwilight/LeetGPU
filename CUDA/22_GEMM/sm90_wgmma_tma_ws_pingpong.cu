...
#pragma unroll
            for (int k = 0; k < K; ++k)
            {
                ...
                int offset_b = j * 1024 + k * 128 + stage * 2048; // j is always 0
...
struct PipelineState
{
    ...
    __device__ __forceinline__ PipelineState &advance(int num_iterations)
    {
        ...
        // How many times number of iterations cross over the stage boundary and end up on a odd number => flipped phase
...
{
public:
    static constexpr int Depth = SequenceDepth_;
    static constexpr int Length = SequenceLength_;
    using BarrierStorage = uint64_t[Depth * Length];

private:
    uint64_t *barrier_base_ptr_;
    PipelineState<Depth> stage_;
    int group_id_;
...
public:
        ...
        : barrier_base_ptr_(smem_ptr), group_id_(group_id)
    {
        ...
        if (warp_idx == 0 && lane_predicate)
        {
            ...
            {
                mbarrier_init(&barrier_base_ptr_[i], 128); // 1 warpgroup 128 threads.
            }
        }
        __syncthreads();
        ...
        if (group_id == 0)
        {
            stage_.set(0, 1); // switch first consumer's phase to 1.
        }
    }
...
public:
    ...
    __device__ __forceinline__ WorkTileInfo get_current_work() const
    {
        ...
        int32_t m = static_cast<int32_t>(current_work_linear_idx_ % grid_dim_m_);
        int32_t n = static_cast<int32_t>(current_work_linear_idx_ / grid_dim_m_);
...
template <int bM, int bN, int bK, int NumThreads, class T, class TC, int epM, int epN, int NumPipe = 1, int base = 0,
          class tmaA, class tmaB>
__global__ void __launch_bounds__(384, 1) wgmma_tma_kernel(const T *A, const T *B, TC *C, int M, int N, int K, float alpha, float beta,
                                                           ...
                                                           const __grid_constant__ tmaB tma_b)
{
    ...
    // producer and consumer mbarrier
    ...
    PipelineState<NumPipe> read_state;  // consumer
    ...
    if (warp_group_role == WarpGroupRole::Consumer1)
    {
        ...
        read_state.advance(num_k_tiles); // Advance 2nd Math WG pipeline states to the end of 1st Math WG
    }
...
extern "C" void solve(const half *A, const half *B, half *C, int M, int N, int K, float alpha, float beta)
{
    ...
    dim3 grid(sm_count);
    int smem_size = int(sizeof(T) * ((blockM + blockN) * blockK * numPipe + epM * epN));
    auto kernel_fptr = wgmma_tma_kernel<blockM, blockN, blockK, num_threads, T, TC, epM, epN, numPipe, base, decltype(tmaA_desc), decltype(tmaB_desc)>;
