...
/**
 * NumPipe == 3 is incorrect, need to fix.
 */
template <int bM, int bN, int bK, int NumThreads, class T, int NumPipe = 1, int base = 0>
__global__ void
sgemm_v4(const T *__restrict__ A, const T *__restrict__ B, float *__restrict__ C, int M, int N, int K)
{
    ...
    constexpr int bN_pad = bN + PAD; // no need?
...
#pragma unroll
        for (int i = 0; i < 8; ++i)
        {
            reg_load_gB[i] = ld_global(tile_gB + i * N + tid);
        }
...
#pragma unroll
            ...
            if (tk == 0)
            {
                ...
                auto tile_gB_write = gB + k_tile_next * bK * N;
#pragma unroll
                for (int i = 0; i < 8; ++i) // 128 / (128 / 8) = 8
                {
                    int row = tid / 8 + i * 16;
...
#pragma unroll
                for (int i = 0; i < 8; ++i)
                {
                    reg_load_gB[i] = ld_global(tile_gB_write + i * N + tid);
                }
            }
        }
    }
...
__global__ void
sgemm_v5(const T *__restrict__ A, const T *__restrict__ B, float *__restrict__ C, int M, int N, int K)
{
    const int tid = threadIdx.x;
    const int warp_id = tid / 32;
    const int lane_id = tid % 32;
...
extern "C" void solve(const float *A, const float *B, float *C, int M, int N, int K)
{
    ...
    if (M == 8192 && K == 6144 && N == 4096)
    {
        ...
        auto kernel_fptr = sgemm_v5<blockM, blockN, blockK, num_threads, T, numPipe, base>;
...
/**
 * nvcc sm80_sgemm_m128n128k8.cu -O3 -arch=sm_80 -lcuda -lcublas -o sm80_sgemm_m128n128k8 && ./sm80_sgemm_m128n128k8
 * gemm success
 * cublas time = 21.695921 ms, TFLPOS = 19.004347, mfu = 0.974582
 * mma time = 22.660568 ms, TFLPOS = 18.195346, mfu = 0.933095
 */
int main()
{
    srand(1234);
